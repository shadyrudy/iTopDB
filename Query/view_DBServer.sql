...
SELECT DISTINCT DBServer_SoftwareInstance.id                                                                        AS id,
                ...
                IF
                    ((
                             FunctionalCI_system_id.finalclass IN
                             ('Middleware', 'DBServer', 'WebServer', 'PCSoftware', 'OtherSoftware')),
                     cast(
                             concat(
                              ...
                              ((
                               ...
                               IF
                                   ((
                                    ...
                                    COALESCE(COALESCE((DBServer_dbserver_id_SoftwareInstance.status = 'inactive'), 0),
                                             0),
                                    IF
                                        ((
                                         ...
                                         COALESCE(COALESCE(
                                                          (Middleware_middleware_id_SoftwareInstance.status = 'inactive'),
                                                          0), 0),
                                         IF
                                             ((
                                                      FunctionalCI_system_id.finalclass IN
                                                      ('Middleware', 'DBServer', 'WebServer', 'PCSoftware',
                                                       'OtherSoftware')),
                                              COALESCE(
                                                      (FunctionalCI_system_id_poly_SoftwareInstance.status = 'inactive'),
                                                      0),
                                              IF
                                                  ((
                                                   ...
                                                   COALESCE(
                                                           (FunctionalCI_system_id_poly_BusinessProcess.status = 'inactive'),
                                                           0),
                                                   IF
                                                       ((
                                                        ...
                                                        COALESCE(
                                                                (FunctionalCI_system_id_poly_ApplicationSolution.status = 'inactive'),
                                                                0),
                                                        COALESCE(
                                                                (FunctionalCI_system_id_poly_PhysicalDevice.status = 'obsolete'),
                                                                0)))))))))                                          AS system_id_obsolescence_flag,
                cast(
                        concat(
                ...
                COALESCE(((
                    ...
                    AND (
                                  SoftwareLicence_softwarelicence_id_Licence.end_date <
                                  date_format((now() - INTERVAL 15 MONTH), '%Y-%m-%d 00:00:00'))),
                         0
        ...
        LEFT JOIN (
            middlewareinstance FunctionalCI_system_id_poly_MiddlewareInstance
                JOIN softwareinstance Middleware_middleware_id_SoftwareInstance ON ((
                    FunctionalCI_system_id_poly_MiddlewareInstance.middleware_id =
                    Middleware_middleware_id_SoftwareInstance.id
                ))) ON ((
