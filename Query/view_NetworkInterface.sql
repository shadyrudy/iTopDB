...
SELECT DISTINCT NetworkInterface.id                                                                                 AS id,
                ...
                ((
                 ...
                 ((
                  ...
                  ((
                   ...
                   COALESCE(COALESCE((ConnectableCI_connectableci_id_PhysicalDevice.status = 'obsolete'), 0),
                            0))))                                                                                   AS obsolescence_flag,
    ...
    LEFT JOIN (
        fiberchannelinterface NetworkInterface_poly_FiberChannelInterface
            ...
            physicaldevice DatacenterDevice_datacenterdevice_id_PhysicalDevice
                ...
                DatacenterDevice_datacenterdevice_id_PhysicalDevice.id =
                DatacenterDevice_datacenterdevice_id_FunctionalCI.id
                ))) ON ((
            NetworkInterface_poly_FiberChannelInterface.datacenterdevice_id =
            DatacenterDevice_datacenterdevice_id_PhysicalDevice.id
            ))) ON ((
...
WHERE ((
    ...
    AND (
           0 <> COALESCE((DatacenterDevice_datacenterdevice_id_PhysicalDevice.finalclass IN
                          ('NetworkDevice', 'Server', 'StorageSystem', 'SANSwitch', 'TapeLibrary', 'NAS',
                           'DatacenterDevice')), 1))
    AND (
           0 <> COALESCE((ConnectableCI_connectableci_id_PhysicalDevice.finalclass IN
                          ('DatacenterDevice', 'NetworkDevice', 'Server', 'PC', 'Printer', 'StorageSystem', 'SANSwitch',
                           'TapeLibrary', 'NAS', 'ConnectableCI')), 1)))
